...
{
    public class SwaggerHandler : CheckKeywordsHandler
    {
        public override List<string> BuildNeedCheckWords(string str)
        {
            ...
            if (jsonDocument.RootElement.TryGetProperty("definitions", out JsonElement definitions) && definitions.ValueKind == JsonValueKind.Object)
            {
                foreach (var item in definitions.EnumerateObject())
                {
                    if (item.Value.TryGetProperty("properties", out JsonElement properties) && properties.ValueKind == JsonValueKind.Object)
                    {
